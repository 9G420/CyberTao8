...
你是本项目的**执行?AI 员工**，负责在 Godot 4.6.1 / GDScript 环境下推进开发任务?
...
遇到架构级别的判断时：先记录你的判断依据，执行最保守的方案，在日志里标注"需?Codex 复审"?
...
4. Logs/Mulerun_Work_Report.md                                ?上一轮精确状?
5. Logs/changelog_v0.1.md（最?5 个版本）                    ?近期变更
```

Logs 目录下还?v1/v2 版本?Snapshot 和旧?Plan 文件，那些是**归档版本，不要引?*?
...
| Boss/遭遇格击败消?Bug 修复（resolve_encounter 三分支） | v0.1.51 | 稳定 |
| 单位精简?主角+伙伴槽系统）+ 英雄存活制胜负判?| v0.1.52 | 稳定 |
| Boss解锁自动传?+ 宝可梦式卡牌战斗过渡 | v0.1.53 | 稳定 |
...
| BFC 瘦身：FloorManager 独立类（多层地图逻辑剥离，BFC 881?91行） | v0.1.76 | 稳定 |
| 3D 单位精灵化（billboard Sprite3D+spritesheet行走动画+程序化敌方图标） | v0.1.77 | 稳定 |
| 商品池扩展（9种商品：加牌/移除?随机crest/最大HP+/原有5种） | v0.1.78 | 稳定 |
| 卡牌战斗层深化（4种新卡牌+2种新敌方行为+2个新遭遇?| v0.1.79 | 稳定 |
| 数值平衡调优（能量虹吸/毒素/反击/脉冲猎手/赛博巫医/赛博彩票?| v0.1.80 | 稳定 |
| 全单位程序化像素风格（BGA宝可梦风格，移除spritesheet?2种生物生成器?| v0.1.81 | 稳定 |
| 2D渲染路径spritesheet移除（BoardView全程序化，修复默认模式BUG?| v0.1.82 | 稳定 |
| 商店 remove_card 手动选牌 UI（替换自动移除最弱牌?| v0.1.83 | 稳定 |
| 卡牌对战拖拽手感优化（本地坐标统一+回弹动画+悬停稳定?| v0.1.84 | 稳定 |
| 3D 棋盘拖拽边界放宽（修复拖拽卡边，接近 2D 自由拖拽?| v0.1.85 | 稳定 |
| 音频听感柔化（BGM/SFX降刺耳：音量下调+波形柔化+噪声减量?| v0.1.86 | 稳定 |
| 外部 BGM 接入（优先加载用?MP3，程序化作为回退?| v0.1.87 | 稳定 |
| 3D 可见性与立体感增强（单位黑块兜底 + 功能格抬?+ 切换炫光?| v0.1.88 | 稳定 |
| 像素美术重设计P0（敌?1~04 GBA风重绘） | v0.1.89 | 稳定 |
...
| 能量系统（每回合3点，成长至上??| v0.1.27/38 | 稳定 |
| 双牌堆系统（抽牌/弃牌/洗牌?| v0.1.27 | 稳定 |
| 3种敌方行为模?+ 意图预告 | v0.1.27 | 稳定 |
| 胜利奖励crest | v0.1.27 | 稳定 |
| 调试快捷按钮（一键测试卡牌战斗） | v0.1.28 | 稳定 |
| 持久牌组系统（跨战斗保留?| v0.1.31 | 稳定 |
| 战斗胜利选牌机制??加入牌组?| v0.1.31 | 稳定 |
| CardRewardPanel 奖励选牌面板 | v0.1.31 | 稳定 |
| 5种遭遇敌方（?种新敌方?| v0.1.32 | 稳定 |
| 5个遭遇格（覆盖棋盘多条路线） | v0.1.32 | 稳定 |
| DeckViewPanel 牌组查看面板 | v0.1.34 | 稳定 |
| 卡牌升级机制?4种牌升级数据+奖励面板双模式） | v0.1.36 | 稳定 |
| Boss 遭遇（零号协?HP20/ATK3/6阶段+独立视觉+增强奖励?| v0.1.37 | 稳定 |
| 能量成长机制（遭遇胜?1/Boss+2，上??| v0.1.38 | 稳定 |

### 2.3 当前牌组数据?4种牌，初?0张，均可升级一次）
...
| 铁壁 | 2E | 减伤4?| 铁壁+ ?减伤6 |
| 吸血?| 2E | 造成3伤害+回复1HP | 吸血? ?4伤害+回复2 |
| 超频修复 | 3E | 回复6HP | 超频修复+ ?回复9 |
| 电弧 | 1E | 造成2伤害+敌方ATK-1 | 电弧+ ?3伤害+ATK-1 |
| 强化斩击 | 1E | 造成4伤害 | 强化斩击+ ?6伤害 |
| 双重防御 | 1E | 减伤3?| 双重防御+ ?减伤4 |
| 毒素注入 | 1E | 施加毒素2回合??回合?| 毒素注入+ ?3回合 |
| 能量虹吸 | 1E | 额外?张牌 | 能量虹吸+ ???|
| 反击 | 1E | 防御+1，蓄力反?伤害 | 反击+ ?防御+2，反? |
| 裂空?| 2E | 3连击?伤害 | 裂空? ?3连击?伤害 |

### 2.4 当前遭遇敌方数据

| 遭遇ID | 名称 | HP | ATK | 行为模式 | 定位 | 位置 |
|--------|------|-----|-----|----------|------|------|
| encounter_01 | 异常哨兵 | 8 | 2 | 攻→攻→防击→重?4回合) | 均衡?| 随机 |
| encounter_02 | 赛博游魂 | 6 | 3 | 攻→重击→攻(3回合) | 爆发?| 随机 |
| encounter_03 | 暗网爬虫 | 12 | 1 | 防击→防击→重击→攻(4回合) | 坦克?| 随机 |
| encounter_04 | 脉冲猎手 | 5 | 4 | 攻→重击→攻(3回合) | 玻璃炮型 | 随机 |
| encounter_05 | 数据幽灵 | 9 | 2 | 攻→防击→重击→重击→攻(5回合) | 长周期型 | 随机 |
| encounter_06 | 量子分裂?| 7 | 2 | 攻→强化→连击→攻→重击(5回合) | 成长?| 随机 |
| encounter_07 | 赛博巫医 | 9 | 2 | 强化→防击→治疗→重击→?5回合) | 持久?| 随机 |
| encounter_boss_01 | 零号协议 | 20 | 3 | 攻→防攻→重击→回复→攻→超?6回合) | Boss | 右上象限随机 |
...
├── FloorManager          ?多层地图管理器（v0.1.76 从BFC剥离?   ~162?
├── DiceManager          ?掷骰 + crest 资源?
├── BoardManager         ?棋盘状态（9个格子字?locked_encounters+portal_cells + BFS?
...
├── CrestActionHandler   ?Crest消耗操作（从BFC剥离?     ~66?
└── CellEffectHandler    ?格子效果处理（从BFC剥离?      ~205?

CardBattleController（卡牌层独立状态机?        ~600?
...
├── UnitRenderer         ?单位渲染（v0.1.60 scale=1.1+等距适配? ~270?
├── IsoTileRenderer      ?等距程序化渲染器（class_name?  ~200??v0.1.61 程序化重?
├── DiceRollAnimation    ?掷骰演出动画（class_name?    ~252??v0.1.49 重写
├── BattleEffects        ?战斗特效静态类（class_name?  ~103??Phase 2 新增
├── DiceDebugPanel       ?棋盘层HUD（含层数显示?      ~540?
├── CardRenderer         ?卡牌渲染静态类（class_name?    ~233??Phase 3 新增
├── CardBattlePanel      ?卡牌战斗UI（v0.1.54 全屏重设计）  ~420?
├── CardRewardPanel      ?奖励选牌/升级面板             ~230?
├── DeckViewPanel        ?牌组查看面板                 ~160?
├── CyberStyle           ?全局视觉风格（class_name注册）~149?
├── CyberBackground      ?背景氛围系统（class_name注册? ~155??Phase 4.1 新增
├── TransitionOverlay    ?宝可梦式百叶窗过渡（CanvasLayer 10?~110??v0.1.53 新增
├── BattleCharRenderer  ?战斗角色立绘渲染（class_name注册?  ~180??v0.1.54 新增
├── UITransitions       ?UI过渡动画工具类（class_name注册?   ~60??v0.1.55 新增
├── UnitPortraitHUD    ?顶部单位头像 HUD（class_name注册?   ~130??v0.1.69 新增
├── PlayerSpriteAnimator ?玩家精灵动画管理器（class_name注册? ~70??v0.1.70 新增 ⚠️ v0.1.82 后已无引用方，可安全删除
├── ShopPanel          ?商店面板（class_name注册?种商品池+crest货币? ~370??v0.1.78 扩展
└── SettingsPanel        ?显示设置

UI3D/（v0.1.71 新增 ?3D 表现层）
├── GridMapper3D        ?格坐标↔3D世界坐标转换（class_name注册? ~40??v0.1.71 新增
├── TileMeshFactory3D   ?格子 BoxMesh 程序化工厂（class_name注册? ~110??v0.1.71 新增
├── UnitMeshFactory3D   ?单位程序化像素精灵工厂（class_name注册，BGA宝可梦风格）  ~400??v0.1.81 全单位程序化重写
└── BoardView3D         ?3D 棋盘视图（SubViewport 内嵌，class_name注册? ~710??v0.1.71 新增 / v0.1.74 反馈系统 / v0.1.81 精灵动画移除

System/
├── DisplaySettings     ?显示设置管理
├── AudioManager        ?音效管理器（class_name注册，多通道SFX+BGM? ~120??v0.1.56 新增
└── SFXGenerator        ?程序化音频引擎（28种音?4种BGM循环?      ~1100??v0.1.56 迁入

Main.gd（场景组?信号中转+音效触发+相机跟随+3D/2D切换?             ~670?
```
...
踩Boss遭遇??_check_encounter() ?encounter_triggered ?卡牌战斗
...
以下规则每一条都有历?bug 教训?*违反会导致运行时崩溃或静默错?*?

```
?不要?:= 于数组字面量、字符串拼接、untyped数组索引
?不要?btn.flat = true（StyleBoxFlat 不渲染）
?不要 await 一个不存在的函数（协程永久挂起?
?Tween 必须?node.create_tween()，不用裸 create_tween()
?不要修改仓库根目录旧项目的任?.gd / .tscn 文件
?不要?ENCOUNTER 阶段以外调用 resolve_encounter()
?新功能必须在 _bootstrap() ?restart_battle() 里都注册
...
- 所有新 UI 组件必须使用 CyberStyle 风格化，不要用裸颜色硬编?
- 面板背景?`CyberStyle.make_panel_bg()`，按钮用 `CyberStyle.style_button(btn, "cyan"/"orange")`
...
| 问题 | 严重程度 | 是否阻塞 | 建议处理时机 |
|------|----------|----------|--------------|
| ~~BUG-001：分辨率/窗口模式切换无效~~ | ~~低~~ | ~~否~~ | ?v0.1.43 已解?|
| ~~BuffManager.tick_turn() 未接入~~ | ~~中~~ | ~~否~~ | ?v0.1.39 已解?|
| ~~BattleFlowController 795行，需瘦身~~ | ~~中~~ | ~~否~~ | ?v0.1.40 已瘦身至588?|
| ~~BattleFlowController 693行（多层地图后增长）~~ | ~~中~~ | ~~否~~ | ?v0.1.76 FloorManager 剥离?791 ?|
| ~~BoardView 640行，职责混杂~~ | ~~中~~ | ~~否~~ | ?v0.1.45 Phase 1 瘦身?23?|
| 电弧?ATK-1 效果仅单场生效（设计缺陷?| ?| ?| 卡牌数据结构重构时修 |
| ~~升级数值未经平衡测试~~ | ~~低~~ | ~~否~~ | ?v0.1.80 首轮数值调优完?|
| 多层地图难度暂不递增（各层敌方数值相同） | ~~低~~ | ~~否~~ | ?v0.1.57 已实现层间难度缩?|
| ~~阵亡单位跨层不复活（可能导致后续层过难）~~ | ~~低~~ | ~~否~~ | ?v0.1.75 已实现复?跨层回复 |
| 复活/回复数值未经平衡测试（REVIVE 50% / HEAL 30%?| ?| ?| 数值调优轮?|
| ~~BUG-002：v0.1.76 回归 ?DiceDebugPanel/Main.gd 直接访问已移?current_floor~~ | ~~高~~ | ~~是~~ | ?v0.1.77 hotfix 已修复（改为 get_current_floor()?|
...
| 任务 | 说明 |
|------|------|
| **像素美术重设?P1（敌?5~07 + Boss?* | 延续 v0.1.89 ?GBA 风格基线，完成敌方全套统一并消除风格混?|

### 🟡 中优先级

| 任务 | 说明 |
|------|------|
| **更多遭遇/Boss** | 丰富战斗多样性，增加中后期内?|

### 🟢 中低优先?

| 任务 | 说明 |
|------|------|
| ~~**SettingsPanel 音量控件**~~ | ~~添加音量滑块 + SFX/BGM 开关~~ ?v0.1.66 已完?|

### ?已完?

| 任务 | 版本 |
|------|------|
| 像素美术重设计P0（敌?1~04 GBA风重绘） | v0.1.89 |
| 3D 可见性与立体感增强（单位黑块兜底 + 功能格抬?+ 切换炫光?| v0.1.88 |
| 外部 BGM 接入（优先加载用?MP3，程序化作为回退?| v0.1.87 |
| 音频听感柔化（BGM/SFX降刺耳：音量下调+波形柔化+噪声减量?| v0.1.86 |
| 3D 棋盘拖拽边界放宽（修复拖拽卡边，接近 2D 自由拖拽?| v0.1.85 |
| 卡牌对战拖拽手感优化（本地坐标统一+回弹动画+悬停稳定?| v0.1.84 |
| 商店 remove_card 手动选牌 UI（替换自动移除最弱牌?| v0.1.83 |
| 全单位程序化像素风格（BGA宝可梦风格，移除spritesheet?2种程序化生物?| v0.1.81 |
...
| 商品池扩展（9种商品：加牌/移除?随机crest/最大HP+/原有5种） | v0.1.78 |
...
| 大世界环境填?缩放+敌方跟随+光标+UI紧凑?| v0.1.63 |
| 镜头跟随优化（选中即居?敌方预告?掷骰动画增强 | v0.1.64 |
| 敌方回合镜头跟随优化（移动跟?延迟切回+柔和过渡?| v0.1.65 |
| 角色形象重构（咩咩启示录风格?音效设置面板 | v0.1.66 |
| 移动逐格行走动画+敌方移动动画+我方回合镜头切回优化 | v0.1.67 |
| 卡牌拖拽出牌+即时伤害反馈 | v0.1.68 |
| 顶部单位头像 HUD | v0.1.69 |
| 玩家角色精灵动画?方向 spritesheet?| v0.1.70 |
| 3D 渐进迁移 P0（BoardView3D+SubViewport+F5切换?| v0.1.71 |
| 3D 交互手感修复（拖?镜头跟随+边界限制+缩放轴心?| v0.1.72 |
| 商店格扩展（ShopPanel独立面板+5种商品池+crest货币+多次购买?| v0.1.73 |
...
| 多层地图?层推?层间奖励+HP保留?| v0.1.42 |
| BUG-001 修复（分辨率/全屏/无边?窗口模式切换?| v0.1.43 |
...
| 美化 Phase 3（CardRenderer+CardBattlePanel 重设?HP?能量点） | v0.1.47 |
| 美化 Phase 4.1（CyberBackground 背景氛围升级?| v0.1.48 |
| 掷骰演出升级（伪3D等距骰子+全屏居中?| v0.1.49 |
| Boss锁定+哨兵前置+传送门机制 | v0.1.50 |
| Boss/遭遇格击败消?Bug 修复 | v0.1.51 |
| 单位精简?主角+伙伴槽系统）| v0.1.52 |
| Boss解锁自动传?宝可梦式过渡 | v0.1.53 |
| 全屏独立卡牌战斗界面+角色立绘+扇形手牌 | v0.1.54 |
| 商店?宝箱格（9种可交互格子?| v0.1.41 |
| BattleFlowController 瘦身?95?88行） | v0.1.40 |
...
| 棋盘随机生成 | v0.1.35 |
| 牌组查看面板 | v0.1.34 |
| DEFEND/SKILL/TRICK crest 消耗入?| v0.1.33 |
...
| 美化 Phase 5：音效系?| AudioManager + 基础音效接入 |
| 美化 Phase 6?.5D 棋盘 | ?v0.1.58 等距贴图棋盘已实?|
...
2. 读上一?Mulerun_Work_Report.md，确认起?
3. 确认任务边界（服务棋盘层还是卡牌层？都不是则不优先做?
3.5 版本号以 git log 实际为准，目标版本顺?
    （如当前?v0.1.50，则 P0 目标?v0.1.51，P1 目标?v0.1.52?
4. 写代?
5. 自查六项闭环是否完好?
   掷骰 / 移动 / 攻击 / 召唤 / 敌方回合 / 胜负重开
   + 遭遇触发 / 卡牌战斗 / 选牌奖励 / HP同步回棋?
6. 更新以下四个日志文件（全部强制，缺一不推送）?
   ?Logs/Mulerun_Work_Report.md        ?本轮精确状?
   ?Logs/changelog_v0.1.md             ?追加版本条目
   ?Logs/AI_Employee_Guide_v3.md       ?同步版本?§2.2完成列表+§6任务优先?
   ?如有架构变化：Logs/CyberTao_Migration_Snapshot_zh_v3.md
7. 提交并推?
8. 聊天里回复一句：已完?v0.1.XX，日志已写入并推?
```

> **⚠️ 日志完整性检?*：推送前自查——Mulerun_Work_Report.md、changelog_v0.1.md、AI_Employee_Guide_v3.md 是否全部在本?commit 中？如果缺少任何一个，**停下来补上再推?*?
...
必须（每轮，缺一不可）：
  ...
  Logs/AI_Employee_Guide_v3.md           ?版本?完成列表+任务优先级（同步?

条件更新（阶段状态明显变化时）：
  Logs/CyberTao_Migration_Snapshot_zh_v3.md
```

> **注意**：AI_Employee_Guide_v3.md ?v0.1.52 起由"条件更新"升级?*每轮强制更新**。每个版本完成后至少同步三处：文件头版本号、?.2 完成列表、? 已完成任务列表。忘记更新等同于未完成任务?
...
?不要把卡牌逻辑写进 BoardView ?DiceDebugPanel
?不要?ENCOUNTER 阶段允许棋盘操作
?不要在一个任务里自行扩大范围
...
| 情况 | 处理方式 |
|------|----------|
| 技?bug | 定位根因，日志写清楚"发现→根因→修复" |
| 设计冲突 | 不自行决定，日志记录冲突，标"需要Codex复审"，选保守方案继?|
| 范围不清?| 先在聊天里澄清，不自行扩?|
| 发现可以重构的地?| 写进日志"建议下一?，不顺手重构 |
| 遇到数值平衡问?| 先实现，在日志Codex复审标注里说明你的判?|
| 任务单与仓库不一?| 发现任务单信息与仓库实际状态不一致（版本?文件路径/函数名等）→ 不要自行假设，先在聊天里说明差异，等用户确认后再执行 |
...
| "生成交接? | 只生?Handoff_Package_latest.md 并推?|
| "更新上岗指令" / "同步指令" | 执行 §14 完整流程 |
| "理解确认" | 输出 §1.2 格式 |
| "自查" | 检查棋?卡牌双层全部闭环 |
| "Codex 复审" | ?Work Report 复审标注字段说明，等待人工介?|

---

## 13. 账号交接机制（积分耗尽前执行）

> 当用户说"执行交接流程"?准备交接"时触发本节?
> **建议在还?200+ 积分时执行，不要等到断开再做?*

### 13.1 为什么不导出对话

对话包含大量噪音：试错过程、反复确认、废弃方案?
真正有价值的上下文全?Git 里。交接的本质是生成一?此刻精确状态的快照"文件?
...
```markdown
# CyberTao: Dice Beast Protocol ?交接?
...
# 更新 Mulerun_Work_Report.md（本轮任务注?执行账号交接"?
...
git commit -m "chore: 生成账号交接?v0.1.XX ?下一账号"
...
```
交接包已生成并推送?
新账号克隆仓库后读取 Logs/AI_Employee_Guide_v3.md ?
Logs/Handoff_Package_latest.md 即可启动?
```

### 13.5 交接质量原则

- **精确优于全面**：交接包?此刻状态快?，不是项目文?
- **下一步要具体**：不?继续开?，要写到文件名和完成标准
- **问题标注影响**：说明是否阻塞下一?
...
> 当用户说"更新上岗指令"?同步指令"时触发本节?

### 14.1 什么情况下需要更新本文件

- 项目完成了新的阶段性里程碑（如完成一?Day 任务?
- 架构发生明显变化（新?删除重要模块?
...
2. 更新本文件（AI_Employee_Guide_v3.md）中以下部分?
   ...
   - §3.1 架构概览中的行数（更新为实际值）
   - §5 技术债清单（标记已解决，追加新债）
   - §6 下一阶段任务优先级（根据最?Work Report 调整?
...
6. 提交?
   ...
   git commit -m "docs: 更新上岗指令 v3 ?v0.1.XX 状?
   git push origin codex/dice-beast-protocol
```
...
| `Handoff_Package_latest.md` | 最新交接包 | 交接时覆?|
| `CyberTao_Migration_Snapshot_zh_v3.md` | 项目全貌+架构 | 阶段性更?|
| `Mulerun_Work_Report.md` | 上一轮精确状?| 每轮覆盖 |
| `changelog_v0.1.md` | 完整版本历史 | 每轮追加 |
| `Board_Card_Battle_Concept_zh.md` | 双层玩法设计文档 | 设计变更?|
| `Demo_Roadmap_2p5D_zh.md` | 中长期路线图 | 阶段性更?|
| `Art_Beautification_Strategy_zh.md` | 美术美化推进策略?阶段?| 美化阶段参?|
