...
| 外部 BGM 接入（优先加载用户 MP3，程序化作为回退） | v0.1.87 | 稳定 |
...
| 毒素注入 | 1E | 施加毒素2回合（2伤/回合） | 毒素注入+ → 3回合 |
| 能量虹吸 | 1E | 额外抽2张牌 | 能量虹吸+ → 抽3张 |
| 反击 | 1E | 防御+1，蓄力反击2伤害 | 反击+ → 防御+2，反击3 |
| 裂空斩 | 2E | 3连击各2伤害 | 裂空斩+ → 3连击各3伤害 |

### 2.4 当前遭遇敌方数据

| 遭遇ID | 名称 | HP | ATK | 行为模式 | 定位 | 位置 |
|--------|------|-----|-----|----------|------|------|
| encounter_01 | 异常哨兵 | 8 | 2 | 攻→攻→防击→重击(4回合) | 均衡型 | 随机 |
| encounter_02 | 赛博游魂 | 6 | 3 | 攻→重击→攻(3回合) | 爆发型 | 随机 |
| encounter_03 | 暗网爬虫 | 12 | 1 | 防击→防击→重击→攻(4回合) | 坦克型 | 随机 |
| encounter_04 | 脉冲猎手 | 5 | 4 | 攻→重击→攻(3回合) | 玻璃炮型 | 随机 |
| encounter_05 | 数据幽灵 | 9 | 2 | 攻→防击→重击→重击→攻(5回合) | 长周期型 | 随机 |
| encounter_06 | 量子分裂体 | 7 | 2 | 攻→强化→连击→攻→重击(5回合) | 成长型 | 随机 |
| encounter_07 | 赛博巫医 | 9 | 2 | 强化→防击→治疗→重击→攻(5回合) | 持久型 | 随机 |
...
CardBattleController（卡牌层独立状态机）         ~600行
...
| ~~升级数值未经平衡测试~~ | ~~低~~ | ~~否~~ | ✅ v0.1.80 首轮数值调优完成 |
...
| 任务 | 说明 |
|------|------|
| **像素美术重设计 P1（敌方05~07 + Boss）** | 延续 v0.1.89 的 GBA 风格基线，完成敌方全套统一并消除风格混搭 |
...
| 任务 | 说明 |
|------|------|
| ~~**SettingsPanel 音量控件**~~ | ~~添加音量滑块 + SFX/BGM 开关~~ ✅ v0.1.66 已完成 |

### ✅ 已完成

| 任务 | 版本 |
|------|------|
| 像素美术重设计P0（敌方01~04 GBA风重绘） | v0.1.89 |
| 3D 可见性与立体感增强（单位黑块兜底 + 功能格抬升 + 切换炫光） | v0.1.88 |
| 外部 BGM 接入（优先加载用户 MP3，程序化作为回退） | v0.1.87 |
...
| 敌方回合镜头跟随优化（移动跟踪+延迟切回+柔和过渡） | v0.1.65 |
