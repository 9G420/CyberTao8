...
- 新增: 召唤单位三职业分化（侦察型/突击型/护卫型），按当前 crest 倾向自动生成，不再是单一模板单位。
- 新增: 召唤单位“战术收益”接入（登场收益 + 每轮指挥收益），让召唤在资源循环里有持续意义。
- 修改: 敌方目标选择加入嘲讽规则，护卫型召唤单位可吸引火力，形成明确保护职责。
- 修改: 护卫型召唤单位为主角提供邻接减伤光环（DEF+1 等效），提升棋盘站位策略价值。
- 修改: 召唤次数与场上上限微调（每层 3 次、场上 2 体），支持“功能搭配”而非一次性功能测试。
