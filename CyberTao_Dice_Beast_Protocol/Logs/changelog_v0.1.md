...
### 修改（棋盘外背景“有画面”）
- `IsoTileRenderer.gd`：外场平台增加可视化内容，不再纯空白
  - 四角赛博面板
  - 左右能量节点环
  - 远景条带装饰
- 保留棋盘主体可读性，外场仅做低强度氛围与区分

## v0.1.101 - 2026-04-02

### 修复（2D棋盘中心与外场观感）
- `BoardView.gd`：选中单位时持续跟随其居中，避免角色跑到边角导致构图怪异
- `BoardView.gd`：选中单位状态下逐步回收拖拽偏移，保持棋盘与角色在画面中区
- `IsoTileRenderer.gd`：外场平台底色提亮并加入轻量警示斜线，强化“棋盘内外区分”

### 说明
- 2D 保持方正稳定优先，不再启用导致歪斜的伪视角旋转
...
### 修复（2D中键无反应 + 棋盘外仍显黑边）
- `BoardView.gd`：2D 视角输入增加兼容路径
  - 中键拖拽仍可调视角
  - 新增 `Alt/Shift + 右键拖拽` 作为备用视角控制（部分鼠标中键事件丢失场景）
- `BoardView.gd`：棋盘层先铺底色，避免外部区域出现纯黑
- `BoardView.gd`：降低边缘暗角强度，减少“黑边包裹感”

### 备注
- 2D 操作现在为：
  - 右键拖拽：平移
  - 中键拖拽：视角偏移
  - Alt/Shift+右键拖拽：视角偏移（兼容）
...
- `BoardView.gd`：新增 2D 中键按住拖拽视角控制
  - 上下拖：俯仰偏移（通过视口中心 Y 偏移模拟）
  - 左右拖：缩放微调
- `BoardView3D.gd`：棋盘重建时新增外环环境地台（ambient pad），消除棋盘外黑边割裂感
- `BoardView3D.gd`：远距单位动态缩放上限继续提高，最远视角更容易看清单位
- `TileMeshFactory3D.gd`：上一版功能格 3D 标识继续沿用，外环地台与主棋盘形成层次对比
...
### 修改（3D 远距可读性 + 中键视角控制 + 功能格立体化）
- `BoardView3D.gd`：新增中键按住拖拽视角功能（Pitch + Yaw）
- `BoardView3D.gd`：右键保留平移；中键独立视角旋转，互不冲突
- `BoardView3D.gd`：远距单位动态缩放增强（最大倍率提升），改善最远距离不可见
- `UnitMeshFactory3D.gd`：精灵基准尺寸微调，配合动态缩放避免近景爆屏
- `TileMeshFactory3D.gd`：功能格新增 3D 标识物（箱体/棱柱/圆柱），不再只是平面色块
...
### 修改（像素美术重设计 P0 - 敌方第一批）
- `UnitMeshFactory3D.gd`：重绘敌方 encounter_01~04 程序化像素形象（GBA风体块比例、配色层级、轮廓高光）
- 第一批完成：
  - `encounter_01` 异常哨兵 → 大头机甲兵风格
  - `encounter_02` 赛博游魂 → 斗篷幽灵风格
  - `encounter_03` 暗网爬虫 → 甲壳蛛风格
  - `encounter_04` 脉冲猎手 → 迅捷利爪兽风格

### 备注
- 本轮只改第一批敌方（01~04），作为整套美术风格基线
- 下一轮将按同风格继续重绘 05~07 + Boss，并同步 UI/卡牌视觉语言
...
### 修改（3D 模式棋盘拖拽自由度修正）
- `BoardView3D.gd`：放宽 `_clamp_drag_offset()` 的拖拽边界策略，修复 3D 视图拖拽到某位置后“卡住/拉不动”的手感问题
- 拖拽最大偏移从旧版固定 `half_board * 0.5`（12x12 下仅 ±6）改为动态边界：`half_board * 2.0 + zoom_extra * 0.8`

### 修复
- **BUG**：3D 模式棋盘拖拽范围过小，无法像 2D 一样自由平移 —— 根因是 v0.1.72 的边界夹紧系数过于保守

### 备注
- 本轮仅调整 3D 相机拖拽边界，不改动 2D BoardView 拖拽逻辑
- 仍保留边界保护，避免相机无限漂移到远离棋盘区域
