...
### 淇敼
- `IsoTileRenderer.gd`锛氬鍦烘柊澧炲浐瀹氳竟妗嗗彴搴э紙鍙屽眰杈规锛変笌鍥涜缁撴瀯浠讹紝寮哄寲鈥滄鐩樿垶鍙扳€濊疆寤?

### 璋冩暣
- 鍙栨秷 2D/3D 鎷栨嫿鍚庣殑鑷姩鍥炴琛屼负锛堟墜鎰熶紭鍖栵紝閬垮厤寮哄埗鎷夊洖锛?
  - `BoardView.gd`锛氱Щ闄ら€変腑鐘舵€佷笅 `_drag_offset` 鑷姩 lerp 鍥為浂
  - `BoardView3D.gd`锛氱Щ闄?`_drag_offset_accumulated` 绌洪棽鑷姩鍥炴

## v0.1.102 - 2026-04-02

### 淇敼锛堟鐩樺鑳屾櫙鈥滄湁鐢婚潰鈥濓級
- `IsoTileRenderer.gd`锛氬鍦哄钩鍙板鍔犲彲瑙嗗寲鍐呭锛屼笉鍐嶇函绌虹櫧
  - 鍥涜璧涘崥闈㈡澘
  - 宸﹀彸鑳介噺鑺傜偣鐜?
  - 杩滄櫙鏉″甫瑁呴グ
- 淇濈暀妫嬬洏涓讳綋鍙鎬э紝澶栧満浠呭仛浣庡己搴︽皼鍥翠笌鍖哄垎

## v0.1.101 - 2026-04-02

### 淇锛?D妫嬬洏涓績涓庡鍦鸿鎰燂級
- `BoardView.gd`锛氶€変腑鍗曚綅鏃舵寔缁窡闅忓叾灞呬腑锛岄伩鍏嶈鑹茶窇鍒拌竟瑙掑鑷存瀯鍥炬€紓
- `BoardView.gd`锛氶€変腑鍗曚綅鐘舵€佷笅閫愭鍥炴敹鎷栨嫿鍋忕Щ锛屼繚鎸佹鐩樹笌瑙掕壊鍦ㄧ敾闈腑鍖?
- `IsoTileRenderer.gd`锛氬鍦哄钩鍙板簳鑹叉彁浜苟鍔犲叆杞婚噺璀︾ず鏂滅嚎锛屽己鍖栤€滄鐩樺唴澶栧尯鍒嗏€?

### 璇存槑
- 2D 淇濇寔鏂规绋冲畾浼樺厛锛屼笉鍐嶅惎鐢ㄥ鑷存鏂滅殑浼瑙掓棆杞?

## v0.1.100 - 2026-04-02

### 淇锛?D妫嬬洏姝枩涓庤嚜鍔ㄥ洖姝ｅ埌濂囨€綅缃級
- `BoardView.gd`锛氱Щ闄?2D 浼瑙掕緭鍏ヨ矾寰勶紙涓敭/Alt+鍙抽敭 orbit锛夛紝鎭㈠绋冲畾绛夎窛鏋勫浘
- `BoardView.gd`锛氬惎鍔ㄦ椂寮哄埗閲嶇疆 `_view_pitch_offset/_view_yaw_offset = 0`
- `BoardView.gd`锛氭嫋鎷介€昏緫鍥炲綊鈥滀粎鍙抽敭骞崇Щ鈥濓紝閬垮厤 orbit 鐘舵€佸共鎵板鑷存鐩樺€炬枩/鍋忕Щ

### 鏍瑰洜
- 2D 浼瑙?offset 涓庤嚜鍔ㄥ洖姝?鐩告満璺熼殢鍙犲姞锛屽鑷存鐩樷€滄鏂?+ 鏉炬墜鍥炲埌濂囨€綅缃€?
