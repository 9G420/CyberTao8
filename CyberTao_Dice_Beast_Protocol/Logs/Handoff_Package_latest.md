...
v0.1.113 已把召唤体系做成 DM 式战术单位：三职业分化、嘲讽护卫与主角减伤光环生效，并接入登场/回合收益，召唤不再是“无意义填格”。
...
1. 把第一章敌方 AI 从“最近点靠近”升级为“卡路/堵路/围压”策略，强化封锁区体感。
2. 继续拆分 `Main.gd` 的纯中转与表现触发逻辑，减轻入口层职责。
3. 用真实 API Key 对 OpenAI 生图链路做一次端到端回归，而不只停留在启动成功。
