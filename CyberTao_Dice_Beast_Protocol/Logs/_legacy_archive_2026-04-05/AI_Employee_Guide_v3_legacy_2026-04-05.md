...
- 当用户目标包含“机制重构 / 玩法重构 / 战斗系统重构”时，开始执行前必须先读：
  - `Logs/Execution_Command_Center.md`
  - `Logs/Mechanic_Refactor_Plan_A.md`
  - `Logs/Handoff_Package_latest.md`
- 若三者冲突，以 `Execution_Command_Center` 的最新覆盖口径为准，并在 `Mulerun_Work_Report` 记录冲突处理。
- 默认执行策略为“方案A渐进重构”，禁止无确认的一次性推倒重做。
# AI Guide Override (Refactor)
...
## 强制规则（重构会话）
- 遇到“机制重构”需求，必须先读：
  1. `Logs/Execution_Command_Center.md`（顶部覆盖段）
  2. `Logs/Mechanic_Refactor_Plan_A.md`
  3. `Logs/Handoff_Package_latest.md`
- 若发现旧条目与重构计划冲突，以执行中枢覆盖段为准。
- 当前唯一主任务口径：`A2_path_loop_resonance`。
