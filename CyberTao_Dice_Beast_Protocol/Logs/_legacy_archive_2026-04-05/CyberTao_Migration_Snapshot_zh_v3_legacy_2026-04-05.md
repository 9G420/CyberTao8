...
v0.1.117-dev 已进入“方案A渐进重构”桥接阶段：命令链核心层已接入战斗流程，玩家关键输入开始统一走 Command 执行入口。
...
| `Project/Scripts/Core/Command.gd` | 命令链原子命令资源（Bridge-1） | 新增 |
| `Project/Scripts/Core/CommandChain.gd` | 玩家命令序列资源（Bridge-1） | 新增 |
| `Project/Scripts/Core/CommandExecutor.gd` | 命令执行器（Bridge-1） | 新增 |
...
- `Project/Scripts/Core/`：重构桥接层（命令链、执行器、后续补丁系统入口）
...
| 命令链桥接仅完成 Bridge-1 | 中 | 否 | 目前是“统一执行入口”，尚未接入路径闭环共鸣与逻辑补丁系统 |
...
1. 执行 `A2_path_loop_resonance`：实现“路径闭环共鸣”最小规则与2D反馈。
2. 执行 `A3_enemy_counterplay`：敌方补齐断环/守点优先级。
3. 执行 `A4_patch_card_bridge`：把现有卡牌逐步映射为命令补丁（先 move/attack/defend）。
4. 继续同步 `Handoff / Work Report / changelog / Snapshot / Mechanic_Refactor_Plan_A` 五份日志。
...
| `Project/Scripts/Core/CommandExecutor.gd` | 命令链桥接执行器 |
...
| `Logs/Mechanic_Refactor_Plan_A.md` | 机制重构主计划（方案A） |
## 快照覆盖口径（Refactor）
...
> 旧段落若与本段冲突，以本段为准。  
> 当前机制推进请优先读 `Execution_Command_Center` 与 `Mechanic_Refactor_Plan_A`。

### 当前结构关键点（精简）
- 玩法主控仍在 `BattleFlowController.gd`（稳定可玩闭环）。
- 桥接层新增 `Project/Scripts/Core/`：
  - `Command.gd`
  - `CommandChain.gd`
  - `CommandExecutor.gd`
